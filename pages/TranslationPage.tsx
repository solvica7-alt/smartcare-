...
const TranslationPage: React.FC = () => {
    ...
    const handleTranslate = async () => {
        ...
        if (!navigator.onLine) {
            ...
            setTranslatedText(t('notConnected'));
            setIsLoading(false);
            return;
        }
    ...
    return (
        <div className="max-w-2xl mx-auto p-4" dir={dir}>
             <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-6 text-center">{t('transTitle')}</h1>
             
             <div className="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden">
                 <div className="p-6 bg-blue-600 text-white flex justify-between items-center">
                     <span className="font-bold text-lg">{t('transTargetLang')}</span>
                     <select 
                        value={targetLang} 
                        onChange={(e) => setTargetLang(e.target.value)}
                        className="bg-blue-700 text-white border-none rounded-md p-2 focus:ring-2 focus:ring-white"
                     >
                         <option value="Arabic">{t('transArabic')}</option>
                         <option value="English">{t('transEnglish')}</option>
                         <option value="Italian">{t('transItalian')}</option>
                     </select>
                 </div>
                 
                 <div className="p-6 space-y-6">
                     <div>
                         <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{t('transOriginalText')}</label>
                     ...
                     >
                         {isLoading ? <LoadingSpinner message=""/> : <><LanguageIcon className="h-5 w-5 me-2"/> {t('transBtn')}</>}
                     </button>

                     {translatedText && (
                         <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-lg p-6">
                             <h3 className="text-sm font-medium text-green-800 dark:text-green-400 mb-2">{t('transResult')}</h3>
